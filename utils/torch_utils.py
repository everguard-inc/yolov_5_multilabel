...
def iou_batch(bb_test, bb_gt):
    bb_gt = torch.unsqueeze(bb_gt, 0)
    bb_test = torch.unsqueeze(bb_test, 1)
...
def predicts_to_multilabel(predicts : torch.tensor, iou_th : float, conf_th : float) -> torch.tensor:
    predicts = predicts[(predicts[...,4]>=conf_th).nonzero(as_tuple=True)[0]]
    iou_matrix = iou_batch(predicts,predicts)
    iou_matrix = torch.triu(iou_matrix,diagonal=1)
    matched_indices = (iou_matrix>iou_th).nonzero(as_tuple=False)
    new_matched_indices = []
    unique = torch.tensor([],device = 'cuda:0' )
    for ids in range(len(matched_indices)):
        if len(unique)==0:
            unique = torch.cat((unique,matched_indices[ids]),0)
        else:
            if matched_indices[ids][0] in unique or  matched_indices[ids][1] in unique:
                unique = torch.cat((unique,matched_indices[ids]),0)
            else:
                new_matched_indices.append(torch.unique(unique).long())
                unique = torch.tensor([],device = 'cuda:0' )
                unique = torch.cat((unique,matched_indices[ids]),0)
    ...
    for ids in new_matched_indices:
        new_pr = predicts[ids]
        new_pr = torch.cat((new_pr[0][:4],new_pr[:,5]))
        new_predicts.append(new_pr.unsqueeze(0))
    return new_predicts
