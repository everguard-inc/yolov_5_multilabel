...
def labels_to_class_weights(labels, nc=80):
    ...
    classes = labels[:, :-4] # labels = [class xywh]
    weights = np.bincount(one_hot_to_list(classes), minlength=nc)  # occurrences per class
...
def labels_to_image_weights(labels, nc=80, class_weights=np.ones(80)):
    ...
    class_counts = np.array([np.bincount(one_hot_to_list(x[:,:-4]), minlength=nc) for x in labels])
...
def scale_coords_1d(img1_shape, coords, img0_shape, ratio_pad=None):
    # Rescale coords (xyxy) from img1_shape to img0_shape
    if ratio_pad is None:  # calculate from img0_shape
        gain = min(img1_shape[0] / img0_shape[0], img1_shape[1] / img0_shape[1])  # gain  = old / new
        pad = (img1_shape[1] - img0_shape[1] * gain) / 2, (img1_shape[0] - img0_shape[0] * gain) / 2  # wh padding
    ...
    coords[[1, 3]] -= pad[1]
    coords[:4] /= gain
    clip_coords_1d(coords, img0_shape)
    return coords
...
def clip_coords_1d(boxes, shape):
    # Clip bounding xyxy bounding boxes to image shape (height, width)
    if isinstance(boxes, torch.Tensor):  # faster individually
        boxes[0].clamp_(0, shape[1])  # x1
        boxes[1].clamp_(0, shape[0])  # y1
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=True,
    ...
    t = time.time()
    if multi_label:
        output = [torch.zeros((0, 1+nc+4), device=prediction.device)] * prediction.shape[0]
    else:
